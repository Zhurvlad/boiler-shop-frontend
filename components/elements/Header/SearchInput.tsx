...
import {$setSearchZIndex, setSearchZIndex} from '../../../context/header';
...
export const SearchInput = () => {
  ...
  const zIndex = useStore($setSearchZIndex)
  ...
  const [onMenuOpenContainersStyles, setOnMenuOpenContainersStyles] = useState({})
  ...
  const searchPart = async (search: string) => {
    try {
      ...
      const names = data.map((i: IBoilerPart) => i.name).map(createSelectOption)
      setOptions(names)
    } catch (e) {
  ...
  return (
    <>
      <div className={styles.header__search__inner}>
        ...
        />
        <span className={styles.header__search__border} ref={borderRef}/>
      </div>
